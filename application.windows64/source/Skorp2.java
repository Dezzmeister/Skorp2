...
public void kill() {
  if (staticTime - inGameTime < 20) {
    ...
    if (staticTime - inGameTime < 35 && dl > 0.7f && spacePresses < 15) {
      //last minute save
      dl += 0.0007f;
    } else {
...
public void eKill() {
  if (staticTime - inGameTime < 20) {
    ...
    if (staticTime - inGameTime < 35 && edl > 0.7f && eSpacePresses < 15) {
      //last minute save
      edl += 0.0007f;
    } else {
      if (staticTime - inGameTime > 50) {
...
public class Rectangle {
  ...
  public void update() {
    if (true) {
      ...
      if (staticTime % 4 == 0 && !modified) {
        diagonal = !diagonal;
        modified = true;
        caughtTime = staticTime;
      }
      
      if (staticTime > caughtTime + 3) {
        modified = false;
      }
  ...
  public char readChar() {
    ...
    for (int i = 65; i < 91; i++) {
      if (keys[i] && keys[SHIFT]) {
        c = PApplet.parseChar(i);
      } else {
        if (keys[i]) {
          c = (PApplet.parseChar(i+32));
        }
      }
    }
    if (keys[BACKSPACE]) {
